...
describe("resolveSkillsNeutral / resolveAllAsSkillsNeutral", () => {
  it("resolveSkillsNeutral includes Hermes audit, provenance, service queue, and quality-gate checks in trellis-check", () => {
    ...
    expect(check?.content).toContain(
      `${py} ./.trellis/scripts/hermes/validate.py --task <task> --kind audit`,
    );
    expect(check?.content).toContain(
      `${py} ./.trellis/scripts/hermes/validate.py --task <task> --kind provenance`,
    );
    expect(check?.content).toContain(
      `${py} ./.trellis/scripts/hermes/validate.py --task <task> --kind service_queue`,
    );
    expect(check?.content).toContain(
      `${py} ./.trellis/scripts/hermes/report.py quality-gate --task <task>`,
    );
    expect(check?.content).toContain("Fix any Hermes failures before proceeding");
  });

  it("resolveCommands requires Hermes quality gate and record validation before finish-work archives tasks", () => {
    const commands = resolveCommands(claudeCtx);
    const finishWork = commands.find((command) => command.name === "finish-work");
    const content = finishWork?.content ?? "";
    const py = getPythonCommandForPlatform();
    const qualityGate =
      `${py} ./.trellis/scripts/hermes/report.py quality-gate --task <task>`;
    const validations = [
    ...
    expect(content).toContain("Do not archive");
    expect(content.indexOf(qualityGate)).toBeLessThan(
      content.indexOf(`${py} ./.trellis/scripts/task.py archive <task-name>`),
    );
    for (const validation of validations) {
      expect(content.indexOf(validation)).toBeLessThan(
        content.indexOf(`${py} ./.trellis/scripts/task.py archive <task-name>`),
      );
    }
  });
