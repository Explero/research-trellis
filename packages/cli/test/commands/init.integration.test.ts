...
describe("init() integration", () => {
  ...
  it("#1 creates expected directory structure with defaults", async () => {
    ...
    // Hermes research overlay is part of the Trellis control plane.
    expect(
      fs.existsSync(path.join(tmpDir, ".trellis", "hermes", "config.yaml")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trellis", "hermes", "state_machine.yaml"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trellis", "hermes", "records", "README.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trellis", "hermes", "records", "recordbus.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".trellis", "hermes", "ledgers", ".gitkeep"),
      ),
    ).toBe(true);
    for (const script of [
      "__init__.py",
      ...
      expect(
        fs.existsSync(
          path.join(tmpDir, ".trellis", "scripts", "hermes", script),
        ),
      ).toBe(true);
    }
    expect(
  ...
  it("#3b codex platform creates skills plus .codex assets", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".agents", "skills", "trellis-tdd", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it("#3c kiro platform creates .kiro/skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "trellis-grill-me", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "trellis-tdd", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it("#3g qoder platform creates .qoder/commands + .qoder/skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".qoder", "skills", "trellis-grill-me", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".qoder", "skills", "trellis-tdd", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it("#3h codebuddy platform creates .codebuddy/commands/trellis", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".codebuddy", "skills", "trellis-tdd", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it("#15 monorepo: bootstrap task references per-package spec paths", async () => {
    ...
    expect(prd).toContain("- [ ] 补充 core 的开发规范");
    expect(prd).toContain("- [ ] 补充 ui 的开发规范");
  ...
  it("#19 init writes bumped hook timeouts (issue #267)", async () => {
    ...
    const hermesPreTool = settings.hooks.PreToolUse.find((entry) =>
      entry.hooks.some((hook) =>
        hook.command.includes("hermes-runtime-guard.py"),
      ),
    );
    expect(hermesPreTool?.matcher).toBe(
      "Edit|Write|MultiEdit|Bash|apply_patch",
    );
    expect(hermesPreTool?.hooks[0].timeout).toBeGreaterThanOrEqual(15);
  });
});
