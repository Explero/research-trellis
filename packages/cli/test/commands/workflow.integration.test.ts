/**
 * Integration tests for `research-trellis workflow` and the init/update hash boundary
 * for non-native workflow selection.
 *
 * Coverage:
 * - `research-trellis workflow --template native`: writes bundled content, keeps hash.
 * - `research-trellis workflow --template tdd`: writes marketplace content, removes hash.
 * - `research-trellis init --workflow tdd`: marketplace content is written, hash removed.
 * - `research-trellis update` after switch to tdd does NOT silently restore native.
...
describe("research-trellis workflow integration", () => {
  let tmpDir: string;
  ...
  it("research-trellis workflow --template native refreshes hash after switching from tdd", async () => {
    stubMarketplaceFetch();
    await init({ yes: true, workflow: "tdd" } as Record<string, unknown>);
    expect(
  ...
  it("research-trellis workflow --template tdd writes marketplace content and removes the hash", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    expect(loadHashes(tmpDir)[PATHS.WORKFLOW_GUIDE_FILE]).toBeTruthy();
  ...
  it("research-trellis update after switching to tdd does not silently restore native workflow", async () => {
    stubMarketplaceFetch();
    await init({ yes: true });
    await runWorkflowCommand({ template: "tdd" });
