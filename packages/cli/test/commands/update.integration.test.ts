...
describe("update() integration", () => {
  ...
  it("#15b adds Hermes overlay templates when they are new to an existing project", async () => {
    await setupProject();
    ...
    const hermesPaths = [
      ...
      ".trellis/scripts/hermes/record.py",
      ".trellis/scripts/hermes/validate.py",
      ".trellis/scripts/hermes/experiment.py",
    ...
    let hashes: Record<string, unknown> = readHashesV2(hashFilePath());
    for (const hermesPath of hermesPaths) {
      fs.rmSync(projectFile(hermesPath), { force: true });
      hashes = removeHashEntry(hashes, hermesPath);
    }
    writeHashesV2(hashFilePath(), hashes as Record<string, string>);

    await update({ force: true });

    for (const hermesPath of hermesPaths) {
      expect(fs.existsSync(projectFile(hermesPath))).toBe(true);
    }
    const updatedHashes = readHashesV2(hashFilePath());
    expect(updatedHashes[".trellis/hermes/config.yaml"]).toBe(
      computeHash(readProjectFile(".trellis/hermes/config.yaml")),
    );
  });
  ...
  it("#22 preserves existing Claude statusLine config and hook file on update", async () => {
    ...
    const hooks = updatedSettings.hooks as {
      PreToolUse?: {
        matcher?: string;
        hooks: { command: string; timeout: number }[];
      }[];
    };
    const hermesPreTool = hooks.PreToolUse?.find((entry) =>
