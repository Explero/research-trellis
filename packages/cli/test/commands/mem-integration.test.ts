...
import { listMemProjects } from "research-trellis-core/mem";
...
let runMem: typeof import("../../src/commands/mem.js").runMem;
...
function getProjectScopeCwd(): string {
  return process.platform === "win32"
    ? nodePath.resolve("..", "mem-int-project")
    : "/tmp/mem-int-project";
}

function encodeClaudeProjectCwd(cwd: string): string {
  return process.platform === "win32"
    ? cwd.replace(/[\\/:_]/g, "-")
    : cwd.replace(/[/_]/g, "-");
}
...
function getProjectDir(): string {
  return nodePath.join(
    CLAUDE_PROJECTS,
    encodeClaudeProjectCwd(getProjectScopeCwd()),
  );
}

function getSessionFile(): string {
  return nodePath.join(getProjectDir(), `${sessionId}.jsonl`);
}
...
function seedClaudeSession(): void {
  writeJsonl(getSessionFile(), [
    {
      type: "user",
      cwd: getProjectScopeCwd(),
...
describe("runMem subcommand integration", () => {
  ...
  beforeEach(async () => {
    nodeFs.mkdirSync(getProjectDir(), { recursive: true });
    seedClaudeSession();
    vi.resetModules();
    runMem = (await import("../../src/commands/mem.js")).runMem;
    logs = [];
    errs = [];
  ...
  it("list: prints the session in cwd-scoped output", () => {
    runMem(["list", "--cwd", getProjectScopeCwd()]);
    const joined = logs.join("\n");
    expect(joined).toContain(getProjectScopeCwd());
    expect(joined).toContain("1 session(s)");
  });

  it("list --json: emits a parseable JSON array", () => {
    runMem(["list", "--cwd", getProjectScopeCwd(), "--json"]);
  ...
  it("search: surfaces the matching session and its hit count", () => {
    runMem(["search", "memory", "--cwd", getProjectScopeCwd()]);
    const joined = logs.join("\n");
  ...
  it("search --json: returns an array of matches with score + excerpts", () => {
    runMem(["search", "memory", "--cwd", getProjectScopeCwd(), "--json"]);
    const parsed = JSON.parse(logs[0] ?? "[]") as unknown;
    expect(Array.isArray(parsed)).toBe(true);
    const arr = parsed as {
    ...
    expect(arr[0]?.session.cwd).toBe(getProjectScopeCwd());
  ...
  it("extract: dumps the cleaned dialogue with role headers", () => {
    runMem(["extract", sessionId, "--cwd", getProjectScopeCwd()]);
    const joined = logs.join("\n");
  ...
  it("extract --json: returns session + turns as parseable JSON", () => {
    runMem(["extract", sessionId, "--cwd", getProjectScopeCwd(), "--json"]);
    const parsed = JSON.parse(logs.join("\n")) as {
      session: { id: string };
  ...
  it("extract --grep filters turns to only those matching the keyword", () => {
    runMem(["extract", sessionId, "--cwd", getProjectScopeCwd(), "--grep", "cache"]);
    const joined = logs.join("\n");
  ...
  it("projects: lists distinct cwds with session counts", () => {
    const projects = listMemProjects({ filter: { platform: "all", limit: 50 } });
    expect(projects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ cwd: getProjectScopeCwd(), sessions: 1 }),
      ]),
    );
    runMem(["projects"]);
    const joined = logs.join("\n");
  ...
  it("projects --json: emits an array of {cwd, sessions, by_platform, ...}", () => {
    ...
    expect(Array.isArray(parsed)).toBe(true);
    const ours = parsed.find((p) => p.cwd === getProjectScopeCwd());
  ...
  it("help command prints usage", () => {
    runMem(["help"]);
    expect(logs.join("\n")).toContain("research-trellis mem");
  });
