...
describe("regression: workflow archive without commit", () => {
  ...
  it("[workflow] Phase 3 marks commit as optional", () => {
    expect(workflowMdTemplate).toContain(
...
describe("regression: current-task path normalization", () => {
  ...
  it("[#240] Codex workflow-state output starts with codex mode, not generic sub-agent notice", () => {
    ...
    expect(ctx).not.toContain("<sub-agent-notice>");
    expect(ctx).toContain("<codex-mode>sub-agent:");
...
`;
  ...
  it("Claude review/implement agent templates use trellis-native TDD references", () => {
    ...
    const agentFiles = [
      ...
      "trellis-check.md",
      "trellis-spec-review.md",
      "trellis-code-review.md",
      "trellis-code-architecture-review.md",
    ];

    for (const relativePath of agentFiles) {
      const content = fs.readFileSync(path.join(templateRoot, relativePath), "utf-8");
      expect(content, `${relativePath} should reference trellis-tdd`).toContain(
        "`trellis-tdd`",
      );
      expect(content, `${relativePath} should not reference local matt-skills path`).not.toContain(
        "matt-skills-main",
      );
    }
  });
  ...
  it("[issue-237] all implement/check agent templates contain recursion guards", () => {
    ...
    const agentFiles = [
      ...
      "claude/agents/trellis-spec-review.md",
      "claude/agents/trellis-code-review.md",
      "claude/agents/trellis-code-architecture-review.md",
  ...
  it("[workflow-v2] --mode phase --platform codex (sub-agent mode) filters out generic before-dev routing", () => {
    ...
    // Codex defaults to sub-agent dispatch, so the legacy
    // spawn-trellis-implement block surfaces without an override.
  ...
  it("[workflow-v2] step 2.1 for codex (sub-agent mode) describes self-loaded agent context, not hook injection", () => {
    ...
    // Codex defaults to sub-agent dispatch, so the [codex-sub-agent]
    // block surfaces without an override.
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb defaults to sub-agent dispatch when config absent", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".trellis/tasks/issue-106");
  ...
  it("[issue-codex-dispatch-mode] resolve_effective_platform namespaces codex into codex-sub-agent / codex-inline", () => {
    ...
    // Invalid mode falls back to default sub-agent rather than passing through.
    expect(result.codex_invalid_mode).toBe("codex-sub-agent");
  ...
  it("[issue-codex-dispatch-mode] codex hook injects <codex-mode> banner reflecting dispatch_mode", () => {
    ...
    // Default (no config.yaml) → sub-agent banner.
    ...
    // Explicit inline → inline banner.
    writeConfigYaml("codex:\n  dispatch_mode: inline\n");
    const inlineRun = JSON.parse(
      runPython(codexHookPath, JSON.stringify({ cwd: tmpDir, session_id: "workflow-a" })),
    ) as { hookSpecificOutput: { additionalContext: string } };
    expect(inlineRun.hookSpecificOutput.additionalContext).toContain(
...
describe("regression: sub-agent context injection fallback (0.5.3)", () => {
  ...
  // Markdown class-1 agent files that must carry the fallback protocol.
  // Claude has extra review-gate agents; Kiro is a JSON file (separate test below).
  const CLASS1_MD_AGENT_FILES: { platform: string; rel: string; agent: "implement" | "check" }[] = [
    ...
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/trellis-spec-review.md", agent: "check" },
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/trellis-code-review.md", agent: "check" },
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/trellis-code-architecture-review.md", agent: "check" },
