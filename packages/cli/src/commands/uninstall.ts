/**
 * `research-trellis uninstall` — remove every file written by `research-trellis init` / `update`
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  if (!options.yes) {
    ...
    // honor the same UX as `research-trellis update` (which also fails closed in
    // that case).
