...
def _hermes_experiment_script(repo_root):
    """Return the local Hermes experiment CLI when the template includes it."""
    script = repo_root / DIR_WORKFLOW / "scripts" / "hermes" / "experiment.py"
    return script if script.is_file() else None


def _task_ref_for_hermes(task_dir, repo_root) -> str:
    """Use a repo-relative task path accepted by hermes/experiment.py."""
    return task_dir.name


def _run_hermes_experiment(repo_root, command: str, task_dir):
    script = _hermes_experiment_script(repo_root)
    if script is None:
        return None
    ...
    return subprocess.run(
        [
            ...
            command,
            "--task",
            _task_ref_for_hermes(task_dir, repo_root),
        ],
        cwd=repo_root,
        capture_output=True,
        text=True,
...
def _print_hermes_process_output(result) -> None:
...
def _task_dir_names(repo_root) -> set[str]:
...
def _created_task_dir(repo_root, before: set[str], args: argparse.Namespace):
    tasks_dir = get_tasks_dir(repo_root)
    if not tasks_dir.is_dir():
        return None

    created = sorted(_task_dir_names(repo_root) - before)
    created = [name for name in created if name != "archive"]
    if len(created) == 1:
        return tasks_dir / created[0]

    slug = getattr(args, "slug", None)
    if slug:
        resolved = resolve_task_dir(slug, repo_root)
        if resolved.is_dir():
            return resolved
...
def _initialize_hermes_experiment(repo_root, task_dir) -> None:
    result = _run_hermes_experiment(repo_root, "init", task_dir)
    if result is None or result.returncode == 0:
        return

    print(
        colored("Warning: Hermes experiment initialization failed", Colors.YELLOW),
        file=sys.stderr,
    )
    _print_hermes_process_output(result)


def _validate_hermes_experiment(repo_root, task_dir) -> bool:
    _initialize_hermes_experiment(repo_root, task_dir)
    result = _run_hermes_experiment(repo_root, "validate", task_dir)
    if result is None:
        return True
    ...
    print(
        colored("Error: Hermes experiment validation failed; task start aborted.", Colors.RED),
        file=sys.stderr,
    )
    _print_hermes_process_output(result)
    return False
...
def cmd_create(args: argparse.Namespace) -> int:
    """Create a task, then initialize Hermes when the local Hermes CLI exists."""
    repo_root = get_repo_root()
    before = _task_dir_names(repo_root)
    result = _cmd_create(args)
    if result == 0:
        task_dir = _created_task_dir(repo_root, before, args)
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not _validate_hermes_experiment(repo_root, full_path):
        return 1
    ...
    if not resolve_context_key():
        print(colored(
            "Error: Session identity not available; task start aborted.",
            Colors.RED,
        ))
        print(colored(
