...
`task.json`（任务状态）是当前状态的唯一事实源。阶段和工作包状态只能通过命令变更；对话里说“已经完成”不会把任务改成 `completed`（已完成）。重要变化追加到 `hermes/task-events.jsonl`（任务事件历史）。
...
`run finished`（运行结束）、`package done`（工作包完成）、`task closed`（任务关闭）和 `claim approved`（主张批准）是不同状态。运行成功只能作为证据，不能代替任务关闭；任务关闭也不能代替论文主张批准。
...
- `package-done` 提示缺少证据：先运行实际验证，再通过 `--evidence` 登记命令、报告或产物引用。
...
- 结果：适度拆解、依赖环、capsule 长度、证据门禁、有限 repair、高风险 amend、三种模式、关闭和归档兼容均有自动化覆盖。
