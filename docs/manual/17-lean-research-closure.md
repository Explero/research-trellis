...
模式都会检查钩子心跳；缺少新鲜钩子时给出明确警告，并继续使用派发、结果、记录和审计协议。`standard`（标准）与 `publication`（发表）的额外关闭条件来自运行清单、证据、统计、主张和人工审批，不来自沙箱或已取消的 Codex 严格执行器。
...
有活动 closure 任务时，主代理的每次用户请求都会收到一条紧凑续接提示：当前阶段、当前工作包、下一动作和 `task.json`（任务状态）路径。新生成的交接会记录任务修订号；只有修订号仍匹配时，恢复提示才会给出 `HANDOFF.md`（交接摘要）路径。旧格式或过期交接只显示警告，任务状态仍以 `task.json` 为准。交接文件全文不会自动注入，也不会默认传给子代理；`task.py finish`（结束会话）只清除会话指针，不会隐式生成交接。
