...
`handoff`（交接）入口会创建一个受限的 `coder:configuration`（配置修改）子代理派发。主代理不直接运行写入命令；子代理只能生成当前任务的 `HANDOFF.md`，其结果确认后由确定性代码写入最终版本。

`HANDOFF.md` 只在代理切换、压缩或清理上下文前、任务阻塞、显式调用和关闭时需要。它记录当前目标、阶段、工作包、已完成结果、改动文件、证据、失败尝试、阻塞和下一动作，但不会修改任务、证据或主张状态。

有活动 closure 任务时，主代理的每次用户请求都会收到一条紧凑续接提示：当前阶段、当前工作包、下一动作和 `task.json`（任务状态）路径。`HANDOFF.md`（交接摘要）会写入对应的任务修订号；只有修订号匹配时才提示读取一次，旧修订的交接会明确标记为过期而不会被使用。接收到 `compact`（上下文压缩）或 `clear`（清理上下文）事件时，会强制重注入任务胶囊；交接刷新也会触发一次重注入。交接文件全文不会自动注入，也不会默认传给子代理。
