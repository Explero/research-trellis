...
这套手册面向第一次把 `Trellis-Hermes`（科研改造版）放进真实科研项目的人。当前仓库版本为 `0.6.0-beta.30`（测试版）；它基于 `mindfold-ai/Trellis`（上游项目）修改，不是上游官方版本，也不代表上游立场。
...
- **实验性**：整个科研改造版仍是测试版；`Hermes`（科研工作流）门禁、代理协作、频道工作代理和容器运行都应先在低风险项目试用。
