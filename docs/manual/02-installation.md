...
```bash
npm install --global research-trellis@0.7.0-beta.0
...
```bash
npm exec --yes --package research-trellis@0.7.0-beta.0 -- research-trellis --version
```
...
版本命令输出 `0.7.0-beta.0`，帮助中能看到 `init`（初始化）、`update`（更新）、`workflow`（工作流）、`channel`（协作频道）和 `mem`（会话记忆）等命令。
...
- 结果：源码包版本为 `0.7.0-beta.0`（测试版），运行时要求与包配置一致。
