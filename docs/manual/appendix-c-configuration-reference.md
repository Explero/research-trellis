...
| 环境变量 | 含义 |
| --- | --- |
| `TRELLIS_HOOKS=0`（关闭钩子） | 明确关闭钩子；派发和关闭时提示当前处于协议约束模式 |
...
会话和运行守卫会在 `.trellis/.runtime/context-firewall/`（防火墙运行目录）写入签名心跳。缺少新鲜心跳时工具会警告，三种 closure 模式仍以结构化派发、结果、运行清单、证据和审计记录判断是否完成。原始跟踪只写入 `.trellis/.runtime/hermes-traces/`（原始跟踪目录），默认不进入模板发布包。
