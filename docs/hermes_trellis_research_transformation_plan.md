...
- 使用 `reviewer:evidence`（证据复核模式）扩展证据审查
...
- `reviewer:evidence`（证据复核模式）不能修改源码、指标、切分或基线。
...
目标：把 `planner`（规划代理）、`researcher`（检索代理）、`coder`（编码代理）、`runner`（运行代理）和 `reviewer`（复核代理）的协作固定成可复用模式。
...
- 角色模板：一个公共契约和五个正式角色；具体工作由 `Profile`（模式）表达。
...
- [x] 角色模板收敛为 planner、researcher、coder、runner、reviewer；证据整理改为确定性工具。
...
- [x] reviewer 记录不能替代 `human/root` approval。
...
- [x] `pnpm --filter research-trellis exec vitest run test/templates/hermes-runtime.test.ts test/templates/trellis.test.ts test/templates/claude.test.ts test/templates/codex.test.ts test/templates/shared-hooks.test.ts`：5 个文件、135 个测试通过。
- [x] `PYTHONPYCACHEPREFIX=/tmp/research-trellis-final-pycache python3 -m py_compile packages/cli/src/templates/trellis/scripts/hermes/*.py packages/cli/src/templates/shared-hooks/hermes-runtime-guard.py`：通过。
- [x] `pnpm --filter research-trellis test`：44 个文件、1259 个测试通过。
- [x] `pnpm --filter research-trellis typecheck`：通过。
- [x] `pnpm --filter research-trellis build`：通过。
